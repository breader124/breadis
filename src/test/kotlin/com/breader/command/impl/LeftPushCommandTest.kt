...
class LeftPushCommandTest {

    private val storage = mockk<Storage>()
    private val command = LeftPushCommand(storage)
    ...
    fun `should return list size after pushing values`() {
        // when
        every { storage.lpush(any(), any()) } returns 3
    ...
    fun `should push only values omitting the key itself`() {
        ...
        // then
        verify { storage.lpush("key", listOf("value1", "value2", "value3")) }
    ...
    fun `should return error if storage throws exception`() {
        // when
        every { storage.lpush(any(), any()) } throws IllegalArgumentException("WRONGTYPE")
